...
### 3.3 Embedding: provider adapter + Batch API, cấu hình như tích hợp LLM
- Embedding KHÔNG chạy local trong RAM. Mỗi tích hợp là một row trong bảng `embedding_integrations` (**provider** + base URL + model + API key mã hóa DEK/KEK + **số chiều vector** + cờ **use_batch**), quản lý qua Settings UI tab "Embedding" — cùng pattern với tích hợp LLM.
- **Provider adapter registry** (`src/core/providers/`): mỗi nhà một adapter khai báo `ProviderSpec` (base URL mặc định, capability `supports_batch`, model presets kèm **danh sách chiều hỗ trợ**) — OpenAI (`dimensions` MRL), Mistral (`output_dimension`), Jina (`task` retrieval.query/passage + `dimensions`), Ollama (self-host, không cần key), và **custom** (OpenAI-compatible generic — tokenrouter và đuôi dài market chạy nhờ lớp này). Backend là nguồn sự thật: UI render form preset từ `GET /embedding-integrations/providers`; thêm provider mới = 1 adapter + 1 dòng đăng ký, không sửa frontend. Chiều KHÔNG fix cứng theo provider — user chọn từ danh sách chiều của model (hoặc "Khác…" tự nhập).
- Lý do Batch API phải viết theo từng nhà: **batch KHÔNG nằm trong chuẩn OpenAI-compatible** — OpenAI dùng `POST /v1/batches` (SDK, `input_file_id`), Mistral dùng `/v1/batch/jobs` (REST riêng), Jina dùng `/v1/batch/embeddings` (inline ≤10k); Ollama/custom không có batch. Batch là **cấu hình opt-in từng integration** (checkbox chỉ hiện khi provider hỗ trợ) — ingestion đi batch path khi cả hai điều kiện thỏa: provider `supports_batch` + integration bật `use_batch`. File chuyển status `batching` (job nằm ở provider, restart server không mất — resume poll từ `batch_meta`), worker poll 30s/lần, hoàn tất → ghép vector với chunk (chunking deterministic theo nội dung file) → insert → ready. Lệch số lượng/thiếu kết quả → failed tường minh, không insert vector sai cặp.
...
- `input_kind` phân biệt ngữ nghĩa query (lúc search, `Retriever.search`) và passage (lúc ingest) — Jina ánh xạ sang `task`; nhà khác bỏ qua. Sync embed gọi thẳng openai SDK (bỏ wrapper `OpenAIEmbeddings` — vốn tokenize tiktoken sai với provider khác).
- Resolve integration active mỗi lần nhúng (cache client theo fingerprint `(base_url, model, api_key, provider)`) → đổi model không cần restart. **Không có credential/fallback env cho embedding**: DB chưa có integration active → gọi embedding raise `EmbeddingConfigError` với thông báo rõ, user phải thêm trong Settings. Duy nhất env `EMBED_DIM` = chiều vector mặc định của DB: DB mới chưa có integration nào dùng nó dựng bảng `chunks` rỗng lúc khởi tạo (đổi chiều mặc định → sửa `.env`).
